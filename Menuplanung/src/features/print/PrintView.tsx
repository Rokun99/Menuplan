...
export const PrintView: React.FC<PrintViewProps> = ({ currentDate, showPlanner, allergenMap }) => {
    ...
    const renderCellContent = (meal: string) => {
        if (!meal) return <span className="text-slate-400">-</span>;
        const allergens = allergenMap.get(meal) || [];
        return (
            <>
                <p>{meal}</p>
                {allergens.length > 0 && (
