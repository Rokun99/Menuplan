...
import { RecipeDatabase } from '../../hooks/useRecipes';
...
export const WeeklyOrderView: React.FC<WeeklyOrderViewProps> = ({ currentDate, showPlanner, recipes }) => {
    const { menuData } = useWeekPlan(currentDate);
    ...
    const handleGenerateOrderList = async () => {
        ...
        setError('');
        try {
            const res = await fetch('/orderMeta.json');
            if (!res.ok) {
                throw new Error(`orderMeta.json konnte nicht geladen werden: ${res.status}`);
            }
            const orderMetaData = await res.json();
            const result = generateOrderList(menuData, allRecipes, weekDates, orderMetaData);
            setOrders(result);
    ...
    const exportToPDF = () => {
        ...
        Object.entries(orders).forEach(([supplierCategory, items]) => {
            const supplierInfo = Object.values(SUPPLIERS).find(s => s.categories.includes(supplierCategory));
            const supplierName = Object.keys(SUPPLIERS).find(key => SUPPLIERS[key].categories.includes(supplierCategory));
            ...
            (doc as any).autoTable({
                startY: yPos,
                head: [['Produkt', 'Menge', 'Einheit', 'Liefertag', 'Notiz']],
                body: items.map(item => [item.product, item.roundedQty, item.unit, item.deliveryDay, item.note || '']),
                theme: 'grid',
    ...
    const exportToCSV = () => {
        let csvContent = "data:text/csv;charset=utf-8,Lieferant,Produkt,Menge,Einheit,Liefertag,Notiz\n";
        Object.entries(orders).forEach(([supplierCategory, items]) => {
            const supplierName = Object.keys(SUPPLIERS).find(key => SUPPLIERS[key].categories.includes(supplierCategory)) || supplierCategory;
            items.forEach(item => {
                const row = [supplierName, item.product, item.roundedQty, item.unit, item.deliveryDay, item.note || ''].join(",");
                csvContent += row + "\n";
            });
        });
                ...
                {isCalculated && orders && Object.keys(orders).length > 0 && (
                    <div className="space-y-8">
                        {Object.entries(SUPPLIERS).map(([supplierName, supplierInfo]) => {
                            ...
                            const groupedByDay = supplierItems.reduce((acc, item) => {
                                (acc[item.deliveryDay] = acc[item.deliveryDay] || []).push(item);
                                return acc;
                            }, {} as Record<string, any[]>);

                            const hasAllergenWarning = supplierItems.some(item => item.allergens && item.allergens.length > 0);
                            ...
                            return (
                                <div key={supplierName} className="bg-white rounded-xl shadow-lg overflow-hidden">
                                    <div className="p-4 bg-slate-50 border-b border-slate-200 flex justify-between items-center">
                                        ...
                                        {hasAllergenWarning && <span className="text-sm font-semibold text-orange-600 bg-orange-100 px-3 py-1 rounded-full">⚠️ Enthält Allergene</span>}
                                    </div>
                                        ...
                                        <table className="min-w-full text-sm">
                                            <thead className="bg-slate-100">
                                                <tr>
                                                    ...
                                                    <th className="px-4 py-2 text-right font-semibold text-slate-600">Menge</th>
                                                    <th className="px-4 py-2 text-left font-semibold text-slate-600">Einheit</th>
                                                    <th className="px-4 py-2 text-left font-semibold text-slate-600">Notiz</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                {deliveryDays.map(day => (
                                                    groupedByDay[day] && (
                                                        <React.Fragment key={day}>
                                                            <tr>
                                                                <td colSpan={4} className="px-4 py-2 bg-blue-50 text-blue-800 font-bold">Lieferung {day}</td>
                                                            </tr>
                                                            {groupedByDay[day].map((item: any, index: number) => (
                                                                <tr key={index} className="border-t border-slate-200 hover:bg-slate-50">
                                                                    ...
                                                                    <td className="px-4 py-2 text-right font-medium text-slate-800">{item.roundedQty}</td>
